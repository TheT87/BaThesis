...
Es steht eine Vielzahl an Software- und Hardwareimplementierungen zur Entscheidung von zugriffkontrollentscheidungen zur  Verfügung.
Jenachdem welche Kriterien auf welche Art und Weise erfüllt werden müssen hängt vom spezifischen Anwendungsfall ab.
...
Gibt an welche Typen von Angriffen ein IDS unter idealen Bedingungen festellen kann.
...
\subsubsection{Warscheinlichkeit falscher Alarme}
Gibt die Warscheinlichkeit das durch ein IDS ausgelöste Alarme durch gutartigen bzw. nicht-schädlichen Netzwerkverkehr verursacht wurden an.\\
...
\subsubsection{Warscheinlichkeit einer Erkennung}
...
\subsubsection{Resistenz des IDS}
Ein signaturbasiertes IDS bzw. der menschliche Administrator hinter dem System weisen Probleme auf die nicht direkt beim Umgang mit verabeitetem Netzwerkverkehr, sondern schon bei der bewussten, unbewussten, oder erzwungenen Entscheidung welcher Netzwerkverkehr überhaupt in Frage kommt, entstehen:
\begin{enumerate}
...
\item{Pakete die zwar nicht bösartig sind aber so konstruiert das siea möglichst viele IDS Signaturen auslösen, überfordern den Administrator oder stören eventuell sogar die Verarbeitung von Paketen generell. }
\item{Ein Angreifer könnte eine Vielzahl harmloserer aber trotzdem noch als schädlich zu deklariende Pakete senden um einen größen Angriff im Netzwerkverkehr zu verschleiern.}
...
\subsubsection{Fähigkeit Ereignisse in Bezug zueinander zu setzen}
...
%3.8 Ability to Identify an Attack This measurement demonstrates how well an IDS can identify the attack that it has detected by labeling each attack with a common name or vulnerability name or by assigning the attack to a category. 
...
Die Warscheinlichkeit einen Angriff zu erkennen variiert mit der Rate an falsch-positiven Meldungen. Bei der Konfiguration eines IDS kann entweder die Falsch-positiv-Rate oder die Erkennungsrate optimiert werden.
In dieser Arbeit will ich mich hautsächlich mit der Häufigkeit falsch-positiven Meldungen und der generellen Übersichtsichlichkeit/Interpretierbarkeit der Meldungen durch einen menschlichen Nutzer beschäftigen.		
