...
Zeile \ref{g_lookup_1} Ordnet einer IP-Adresse zu einem geografischen Ort zu.\\
%Möglich aufgrund des Vergabeverfahrens.
Zeile \ref{g_lookup_2} - \ref{g_lookup_3} Ermittelt die Ortszeit am Ursprung der Anfrage mithilfe der lokalen Uhrzeit in Kombination mit dem aus dem Abstand ermittelten Zeitunterschied.\\
Zeile \ref{g_lookup_4} Passt des Grenzwertes entsprechend an.\\\pagebreak
\lstset{escapeinside={(*@}{@*)}}
...
\begin{lstlisting}[firstnumber=53,label={Code_5},linerange={53-64,65-73},caption={Überprüfung der Verbindungsziele eines Endgerätes},numberblanklines=false]
@load site/packages/zeek-agent-v2
@load site/packages/zeek-agent-v2/framework/main
@load site/packages/zeek-agent-v2/table
@load site/zeek-agent-v2
